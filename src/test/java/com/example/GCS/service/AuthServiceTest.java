...
class AuthServiceTest {
    ...
    @Mock
    private VerifyResponseBuilder verifyResponseBuilder;
...
//    }
    ...
    void setUp() {
        ...
        authService = new AuthService(userRepository,firebaseAuth,verifyFirebaseToken,verifyResponseBuilder);
    }
    ...
    void verifyToken_Normal() {
        ...
        verifyResponseBuilder.success(true).build();

        // テスト対象メソッドの実行
        VerifyResponseBuilder verifyToken = authService.verifyToken(idToken);
    ...
    @Test
    void verifyTokenThatInvalidToken() throws FirebaseAuthException {

        String idToken = "invalid";
        verifyResponseBuilder.success(false).addError("JWTトークンが不正です");
        VerifyResponseBuilder verifyResponseBuilder  = mock(VerifyResponseBuilder.class);

        // 例外でモック
        when(verifyFirebaseToken.verifyFirebaseToken(idToken)).thenThrow(new RuntimeException("JWTトークンが不正です"));

        // When（テスト対象の処理を実行）
        VerifyResponseBuilder result = authService.verifyToken(idToken);

        // Then（結果の検証）
        assertFalse(result.build());
